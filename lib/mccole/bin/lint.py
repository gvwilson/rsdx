...
def _lint_contents_again_keys(args, config, content):
    """Check contents against chapter and appendix keys."""
...
def _lint_slug_format(args, config, content):
    ...
    for filepath, doc in content["html"].items():
        slug = "@root" if len(filepath.parts) == 2 else filepath.parts[-2]
        ...
        if slug not in util.get_lint(config).get("disable_h2_id", set()):
...
def collect_bib_keys():
    ...
    return list(m for m in util.BIB_KEY.findall(content))
...
def get_svg_size(svg):
    ...
    if result[0].endswith("px"):
        ...
        result = ("raw", float(result[0]), float(result[1]))
    return result
